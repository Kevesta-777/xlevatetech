...
export const EnhancedIndustryCard = ({ industry, index }: EnhancedIndustryCardProps) => {
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden">
          <div
            className={`relative h-full bg-gradient-to-br ${industry.gradient} opacity-90 backdrop-blur-sm border border-white/20 rounded-2xl overflow-hidden transition-all duration-500 ease-out transform ${
              isHovered ? 'scale-105 shadow-2xl shadow-black/20' : 'shadow-lg'
            }`}
          >
            {/* High-Resolution Background Image */}
            <div className="absolute inset-0 overflow-hidden rounded-2xl">
              <img 
                src={industry.highResImage} 
              ...
              <div className="mt-auto pt-4 border-t border-white/10">
                <div className="flex items-center justify-between mb-3">
                  <button
                    ...
                    className="flex items-center gap-2 text-white/90 hover:text-white text-sm font-medium transition-colors duration-200"
                  >
                    View Clients
                ...
                <Link
                  ...
                  className="w-full bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-6 py-3 rounded-lg text-sm font-medium transition-all duration-200 border border-white/20 flex items-center justify-center gap-2 min-h-[44px]"
                >
                  Learn More
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180">
          <div
            className={`relative h-full bg-gradient-to-br ${industry.gradient} opacity-90 backdrop-blur-sm border border-white/20 rounded-2xl p-6 transition-all duration-500 ease-out transform ${
              isHovered ? 'scale-105 shadow-2xl shadow-black/20' : 'shadow-lg'
            }`}
          >
            ...
            <div className="relative z-10 h-full flex flex-col">
              ...
              <div className="grid grid-cols-1 gap-3 flex-1">
                {industry.exampleClients.map((client, i) => (
                  <div
                    key={i}
                    className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20 transition-all duration-200 hover:bg-white/20 animate-fade-in"
              ...
              <div className="mt-auto pt-4 border-t border-white/10">
                <Link
                  to="/contact"
                  className="w-full bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 border border-white/20 flex items-center justify-center gap-2 min-h-[44px]"
                >
                  Get Custom Quote
                  <ArrowRight className="h-4 w-4" />
                </Link>
